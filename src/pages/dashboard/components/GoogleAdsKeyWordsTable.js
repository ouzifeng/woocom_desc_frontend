...
export default function GoogleAdsKeywordsTable({ startDate, endDate }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/analytics/dashboard/ads-keywords?${params}`, { headers });
  ...
  const totalSessions = keywords.reduce((sum, k) => sum + k.sessions, 0);
            ...
            <TableBody>
              {keywords.map((row, i) => (
                <TableRow key={`${row.keyword}-${i}`}>
                  <TableCell>{row.keyword || '(not set)'}</TableCell>
